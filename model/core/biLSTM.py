...
class biLSTM(object):
    ...
    def classification(self, hidden_states, label_embeddings, hidden_dim, label_embedding_dim, seq_len):
        # hidden_dim here is twice of original hidden_dim
        # hidden_states: [batch_size, max_seq_len, hidden_dim]
        # label_embeddings: [batch_size, label_embedding_dim]
        # seq_len: [batch_size]
        #
    ...
    def build_model(self):
        ...
        fw_lstm = tf.contrib.rnn.BasicLSTMCell(self.hidden_dim)
        bw_lstm = tf.contrib.rnn.BasicLSTMCell(self.hidden_dim)
        ...
        # outputs: [batch_size, max_seq_len, 2*hidden_dim]
        # label_embeddings: [batch_size, label_embedding_dim]
        y_ = self.classification(outputs, self.label_embeddings, 2*self.hidden_dim, self.label_embedding_dim, self.seqlen)
        y_pre = y_
        y_tar = y
