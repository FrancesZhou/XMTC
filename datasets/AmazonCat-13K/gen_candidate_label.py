...
train_asins_file = path + 'train_map.txt'
test_asins_file = path + 'test_map.txt'
...
def get_candidate_labels():
    train_asins = load_txt(train_asins_file)
    test_asins = load_txt(test_asins_file)
    ...
    # train data
    for i in range(len(train_asins)):
        pid = train_asins[i].strip()
        pid = int(pid)
    ...
    for i in range(len(test_asins)):
        pid = test_asins[i].strip()
        pid = int(pid)
